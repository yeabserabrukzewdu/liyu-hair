      ...
      {showText && layout !== 'mark-only' && (
        <div className={`flex flex-col ${layout === 'vertical' ? 'items-center' : 'items-start'}`}>
          <div className="flex items-baseline gap-1.5">
            <span
              className={`font-serif italic font-normal tracking-[0.05em] leading-none ${currentSize.text} ${textColorClass}`}
              style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
            >
              Liyu
            </span>
            <span
              className={`font-sans font-bold tracking-[0.22em] uppercase leading-none ${currentSize.sub} ${textColorClass}`}
            >
              HAIR
            </span>
          </div>
          <span
            className={`font-sans tracking-[0.35em] uppercase text-[7px] md:text-[8px] mt-0.5 font-light ${subColorClass}`}
          >
